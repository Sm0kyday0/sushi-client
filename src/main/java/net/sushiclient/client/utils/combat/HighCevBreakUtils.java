...
public class HighCevBreakUtils {

    private static HighCevBreakAttack find(EntityPlayer player, EntityPlayer target, BlockPos pos, double enemy, double self, double ratio) {
        ...
        return new HighCevBreakAttack(crystalPos, obsidianPos, player, target, placed, damage, placed != null, obsidianPlaced);
    }

    public static List<HighCevBreakAttack> find(EntityPlayer player, EntityPlayer target, double damage, double self, double ratio) {
        BlockPos headPos = new BlockPos(
            target.posX,
            Math.floor(target.posY + target.getEyeHeight() + 2.0),
            target.posZ
        );

        ArrayList<HighCevBreakAttack> result = new ArrayList<>();

        HighCevBreakAttack attack = find(player, target, headPos, damage, self, ratio);
        if (attack != null) {
            result.add(attack);
        }
    ...
    public static List<HighCevBreakAttack> find(EntityPlayer player, double damage, double self, double ratio) {
        ArrayList<HighCevBreakAttack> result = new ArrayList<>();
